...
HDR = "DECLARACIàN PROVISIONAL O DEFINITIVA DE IMPUESTOS FEDERALES"
...
def _parse_period_line(line: str) -> tuple[str | None, int | None, str | None]:
    # "Per¡odo de la declaraci¢n: Noviembre Ejercicio: 2025"
    m = re.search(
        r"Per[i¡]odo\s*de\s*la\s*declaraci[o¢]n\s*:\s*([A-Za-zµÖàé¥ ‚¡¢£¤]+)\s+Ejercicio\s*:\s*(\d{4})",
        line,
        flags=re.IGNORECASE,
    )
    if not m:
...
def _split_sections(raw: str) -> list[str]:
    ...
    # la £ltima ocurrencia de "CANTIDAD A PAGAR" previa al HDR dentro de una ventana.
    ...
    for i, hdr_start in enumerate(idxs):
        ...
        if cap != -1:
            # un peque¤o margen para capturar "TOTAL DE CONTRIBUCIONES" que suele ir arriba
            start = max(0, cap - 250)
...
def parse_sat_declaracion_summary(text: str) -> dict[str, Any]:
    """Extrae un resumen estructurado de un PDF/acuse de declaraci¢n SAT.

    Este acuse suele contener varias secciones (p.ej. ISR e IVA), cada una con:
      - Encabezado HDR
      - L¡nea del impuesto (ej. "ISR PERSONAS FÖSICAS PLATAFORMAS TECNOLOGàGICAS")
      - RFC / Nombre / Tipo / Periodo / N£mero operaci¢n / Fecha presentaci¢n
      - Importes: CANTIDAD A PAGAR / TOTAL DE CONTRIBUCIONES / etc.

    Regresa:
      - campos generales (RFC/periodo/operaci¢n/fecha)
    ...
    for sec in sections:
        ...
        # impuesto = primera l¡nea despu‚s del HDR
        try:
            hdr_i = lines.index(HDR)
        except ValueError:
            hdr_i = 0
        # Busca la primera l¡nea "en may£sculas" despu‚s del hdr
        ...
        for ln in lines:
            ...
            if nln.upper().startswith("RFC:"):
                m = re.search(r"RFC:\s*([A-Z&¥]{3,4}\d{6}[A-Z0-9]{3})", nln, flags=re.IGNORECASE)
                if m:
            ...
            if "Tipo de declaraci¢n" in ln or "Tipo de declaracion" in nln:
                m = re.search(r"Tipo\s*de\s*declaraci[o¢]n\s*:\s*(.+)", ln, flags=re.IGNORECASE)
                if m:
                    sec_obj["tipo_declaracion"] = m.group(1).strip()

            if "Per¡odo de la declaraci¢n" in ln or "Periodo de la declaracion" in nln:
                mes, year, periodo = _parse_period_line(ln)
                sec_obj["periodo_mes"] = mes
                sec_obj["ejercicio"] = year
                sec_obj["periodo"] = periodo

            if "N£mero de operaci¢n" in ln or "Numero de operacion" in nln:
                m = re.search(r"N[u£]mero\s*de\s*operaci[o¢]n\s*:\s*(\d{6,})", ln, flags=re.IGNORECASE)
                if m:
                    sec_obj["numero_operacion"] = m.group(1)

                m = re.search(
                    r"Fecha\s*(?:y\s*hora\s*)?de\s*presentaci[o¢]n\s*:\s*(\d{2}/\d{2}/\d{4})\s*(\d{2}:\d{2})",
                    ln,
                    flags=re.IGNORECASE,
                )
                if m:
            ...
            if "L¡nea de captura" in ln or "Linea de captura" in nln:
                m = re.search(r"(?:L[i¡]nea\s*de\s*captura)\s*:?\s*([0-9]{18,30})", ln, flags=re.IGNORECASE)
                if m:
    ...
    # Deduplicate repeated sections (mismo impuesto + mismo n£mero de operaci¢n)
    ...
    # Campos de c lculo "dentro" del acuse (si aparecen)
    # Nota: estos suelen estar en la primera hoja antes del encabezado, y ayudan a conciliar contra tu c lculo.
